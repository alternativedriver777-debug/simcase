...
- Во вкладке «Предметы» доступно редактирование названия, редкости, веса, изображения и описания с массовым сохранением.
- Быстрая авто-нормализация диапазонов редкостей по всему ролл-диапазону.
- Очистка инвентаря с подтверждением из UI.
- Переключаемая светлая/тёмная тема (с сохранением в настройках).
- Выбор звукового файла для редкостей через системный диалог проводника.
- Визуальные эффекты редкости (например `neon`) применяются в результатах открытия.
...
- `simcase/storage.py` — разделённое хранилище: отдельные JSON-файлы для редкостей, предметов и инвентаря.
